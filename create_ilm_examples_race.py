...
def randomly_mask_document(
  ...
  for i in range(num_examples):
    ...
    while num_retries < max_num_retries and mask is None:
      try:
        mask = tuple(masker.mask(doc,race_list))
...
#randomly mask dataset function modified for race data masking.
def randomly_mask_dataset(
    docs,
    masker,
    num_examples_per_document,
  ...
  print("masking race dataset")
  num_retries_total = 0
  for i in range(len(docs["sentences"])):
    doc = docs["sentences"][i]["text"]
    race_list = docs["sentences"][i]["mentions"]
...
if __name__ == '__main__':
  ...
  parser.set_defaults(
      seed=None,
      data_name='legal_data',
      # data_dir="/media/anamika/DATA/WPI_COURSES/NLP-DR/ilm/data/raw_data/legal_data",
      # data_split='train',
      # out_dir = "/media/anamika/DATA/WPI_COURSES/NLP-DR/ilm/data/out_data",
      mask_cls='ilm.mask.custom.MaskRace',
      mask_arg0=None,
      max_num_documents=None,
      num_examples_per_document=20,
  ...
  print("max_num_retries_per_example ++++++++++++++++++++++++++", args.max_num_retries_per_example)
  ...
  print("masking data started+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++", args.data_split)
  # Create examples
  ...
  print("masking data completed+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++")

  print(len(masked_data))

  # with open("/media/anamika/DATA/WPI_COURSES/NLP-DR/ilm/data/char_masks/out_data/legal-temp.txt", 'w') as f:
  ...
  with open(os.path.join(args.out_dir, '{}.txt'.format(args.tag)), 'w') as f:
    f.write(str(masked_data))
