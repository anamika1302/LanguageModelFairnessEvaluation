...
#   def mask(self, doc,race_list):
#     # print("race_list detected")
#     from nltk import pos_tag
#     masked_spans = []
...
#           off = race_list[j][0]
#           len_tok = race_list[j][1] - off
#           masked_spans.append((MaskProperNounType.RACE, off, len_tok))
#     return masked_spans
...
#race masking code
class MaskRaceType(Enum):
  RACE = 0

class MaskRace(MaskFn):
  def __init__(self, p=1.):
    try:
      pos_tag(['Ensure', 'tagger'])
    except:
  ...
  def mask(self, doc,race_list):
    ...
    if len(race_list) > 0:
      for j in range(len(race_list)):
          off = race_list[j][0]
          len_tok = race_list[j][1] - off
          masked_spans.append((MaskRaceType.RACE, off, len_tok))
    return masked_spans
