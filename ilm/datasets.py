...
def get_dataset(dataset, split, *args, data_dir=None, shuffle=False, limit=None, **kwargs):
  ...
  elif dataset == Dataset.LEGAL_DATA:
    assert split in ['train', 'valid', 'test']
    if data_dir is None:
      data_dir = os.path.join(RAW_DATA_DIR, 'legal_data')
    d = custom_race(split, data_dir=data_dir)
...
def custom_race(split, data_dir):
  fp = os.path.join(data_dir, '{}.json'.format(split))
  try:
    with open(fp, 'r') as f:
      data = json.load(f)
  except:
